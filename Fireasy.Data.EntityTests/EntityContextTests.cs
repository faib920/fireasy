...
WHERE (t0.City = 'London')").ToList();
        ...
        {
            var d = db.Orders.Include(s => s.Customers).Get(10259);
            Console.WriteLine(EntityProxy.Get(d).State);
            Console.WriteLine(EntityProxy.Get(d.Customers).State);
            Assert.AreEqual("CENTC", d.Customers.CustomerID);
        }
        ...
        private void Test<T>() where T : IIntegralQueryable
        {
            var set = db.Set<T>().Where(s => s.Name == "");
