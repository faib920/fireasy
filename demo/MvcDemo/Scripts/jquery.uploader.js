(function ($) {
	...
	$._upload = function (element, options) {
	    ...
	    element.each(function (i, e) {
	        ...
	        var loadcallback = function () {
	            ...
	            if (myFrame) {
	                ...
	                if (myFrame.contentWindow && myFrame.contentWindow.document.body) {
	                    response = myFrame.contentWindow.document.body.innerText;
	                } else if (myFrame.contentDocument && myFrame.contentDocument.document.body) {
	                    response = myFrame.contentDocument.document.body.innerText;
	                }
	        ...
	        form.append(input);
	        input.prop('title', '点击上传').css({ 'width': e.width() + 'px', height: e.height() + 'px' });
	        ...
	        checkExt: function (index, element) {
	            ...
	            if (options.extExclude != null) {
	                ...
	                if ($.inArray(ext, extList) >= 0) {
	                    options.onFileExtError(file, ext, index, element);
	                    return false;
	                }
	            }
