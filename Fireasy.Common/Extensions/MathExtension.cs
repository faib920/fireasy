...
namespace Fireasy.Common.Extensions
{
    ...
    public static class MathExtension
    {
        ...
        /// <returns></returns>
        public static double Round<T>(this T value, int decimals = 2, RoundType roundType = RoundType.FourFive) where T : IConvertible
        {
        ...
        /// <param name="weightFunc">加权函数。第一个参数 <typeparam name="T"></typeparam> 为当前的数值，第二个参数 double 为数组的平均值。</param>
        /// <returns></returns>
        public static double Variance<T>(this T[] array, Func<T, double, double> weightFunc = null) where T : IComparable<T>, IConvertible
        {
            ...
            var sum = 0.0;
            var average = array.Average(s => s.ToDouble(null));
            foreach (var t in array)
            {
                var d = t.ToDouble(null) - average;
                if (weightFunc != null)
                {
