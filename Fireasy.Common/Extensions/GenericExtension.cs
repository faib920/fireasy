...
namespace Fireasy.Common.Extensions
{
    ...
    public static class GenericExtension
    {
        ...
        private static Delegate BuildCloneToDelegate(object obj, Type conversionType)
        {
            var sourceType = obj.GetType();
            ...
            if (@dynamic != null)
            {
                GetDynamicMemberBindings(@dynamic, conversionType, parExp, bindings);
            }
            else
            {
                GetGeneralMemberBindings(obj, sourceType, conversionType, parExp, bindings);
            }
#else
            GetGeneralMemberBindings(obj, sourceType, conversionType, parExp, bindings);
#endif
...
#if !N35
        private static void GetDynamicMemberBindings(IDynamicMetaObjectProvider @dynamic, Type conversionType, ParameterExpression parExp, List<MemberBinding> bindings)
        {
...
#endif
        private static void GetGeneralMemberBindings(object obj, Type sourceType, Type conversionType, ParameterExpression parExp, List<MemberBinding> bindings)
        {
            ...
            foreach (var property in sourceType.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                try
                {
                    ...
                    var descProperty = conversionType.GetProperty(property.Name, BindingFlags.Public | BindingFlags.Instance);
                    if (descProperty == null || !descProperty.CanWrite || !property.CanRead)
                    {
                        continue;
                    }

                    var exp = (Expression)Expression.MakeMemberAccess(parExp, property);
                    if (property.PropertyType != descProperty.PropertyType)
                    {
                        exp = Expression.Call(null, MthToType,
                            Expression.Convert(exp, typeof(object)),
                            Expression.Constant(descProperty.PropertyType),
                            Expression.Constant(null));
                        exp = Expression.Convert(exp, descProperty.PropertyType);
                    }
