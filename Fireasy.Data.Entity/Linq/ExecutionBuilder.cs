...
namespace Fireasy.Data.Entity.Linq
{
    ...
    public class ExecutionBuilder : DbExpressionVisitor
    {
        ...
        private static MethodInfo MthNewPropertyValue = typeof(PropertyValueHelper).GetMethods(BindingFlags.Static | BindingFlags.NonPublic).FirstOrDefault(s => s.Name == "NewValue" && s.IsGenericMethod);
        ...
        private static PropertyInfo ProGetProvider = typeof(IDatabase).GetProperty("Provider");
            ...
            mbmInitExp.Bindings.ForEach(s =>
                {
                    ...
                    if (assign != null)
                    {
                        properties.Add(Expression.Constant(PropertyUnity.GetProperty(mbmInitExp.Type, assign.Member.Name)));
                        var mthNewValue = MthNewPropertyValue.MakeGenericMethod(assign.Expression.Type);
                        var pValue = Expression.Call(null, mthNewValue, Visit(assign.Expression));
                        values.Add(pValue);
                    }
                });
        ...
        protected override Expression VisitConstant(ConstantExpression constExp)
        {
            ...
            {
                var pv = constExp.Value as PropertyValue;
                if (!pv.IsNullOrEmpty())
                {
                    var value = pv.GetStorageValue();
            ...
            IEntityPersistentInstanceContainer instanceContainer, IEntityPersistentEnvironment environment)
        {
            ...
            for (var i = 0; i < properties.Length; i++)
            {
                if (entityEx != null)
                {
                    entityEx.InitializateValue(properties[i], values[i]);
                }
                else
                {
                    entity.InternalSetValue(properties[i], values[i]);
                }
            }
            ...
            if (entityEx != null)
            {
                entityEx.SetState(EntityState.Unchanged);
            }
        ...
        private static int GenerateIdentityValue(IDatabase database, IEntity entity, IProperty property)
        {
            ...
            entity.InternalSetValue(property, PropertyValue.New(value, property.Type));
