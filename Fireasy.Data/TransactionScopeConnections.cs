...
namespace Fireasy.Data
{
    ...
    {
        private static readonly ConcurrentDictionary<Transaction, ConcurrentDictionary<string, DbConnection>> m_transConns =
        ...
        public static DbConnection GetConnection(IDatabase database)
        {
            ...
            if (!m_transConns.TryGetValue(curTrans, out connDictionary))
            {
                connDictionary = new ConcurrentDictionary<string, DbConnection>();
                m_transConns.TryAdd(curTrans, connDictionary);
        ...
        {
            ConcurrentDictionary<string, DbConnection> connDictionary;
            if (!m_transConns.TryGetValue(e.Transaction, out connDictionary))
            {
                return;
            }
            ...
            m_transConns.TryRemove(e.Transaction, out connDictionary);
        }
    }
}
