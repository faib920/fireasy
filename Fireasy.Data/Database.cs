...
namespace Fireasy.Data
{
    ...
    public class Database : IDatabase
    {
            ...
            return UsingConnection(() =>
                {
                    using (var command = CreateDbCommand(queryCommand, parameters))
                    {
                        ...
                        {
                            object result = null;
                            OnCommandExecuted(command, null, parameters, TimeWatcher.Watch(() => result = command.ExecuteScalar()));
            ...
            UsingConnection(() =>
                {
                    ...
                    if (ConnectionString.IsTracking && Track != null)
                    {
                        Track.Write(adapter.InsertCommand, period);
                    }
                }, false);
            ...
            return await UsingConnection(() =>
                {
                    using (var command = CreateDbCommand(queryCommand, parameters))
                    {
                        try
                        {
                            ...
                            OnCommandExecuted(command, null, parameters, TimeWatcher.Watch(() => result = command.ExecuteNonQueryAsync()));
                            command.SyncParameters(parameters);
                            command.ClearParameters();
                            return result;
                        }
        ...
        {
            return await ExecuteScalarAsync(queryCommand, parameters).ContinueWith<T>(t => t.Result == DBNull.Value ? default(T) : t.Result.To<T>());
        }
        ...
        /// <param name="segment">数据分段对象。</param>
        /// <param name="parameters">查询参数集合。</param>
        /// <param name="period">执行所耗用的时间。</param>
        protected virtual void OnCommandExecuted(IDbCommand command, IDataSegment segment, ParameterCollection parameters, TimeSpan period)
        {
        ...
        private DbCommand CreateDbCommand(IQueryCommand queryCommand, IEnumerable<Parameter> parameters)
        {
            ...
            command.CommandText = FormatCommandText(queryCommand.ToString());
        ...
        private void ProcessAdapterTableMapping(IDataAdapter adapter, string tableName)
        {
        ...
        private void ProcessSegmentCommand(CommandContext context, Action acTrue, Action acFalse = null, Action acNosegment = null)
        {
            //使用数据分段
                ...
                context.Command.CommandType == CommandType.Text)
            {
                ...
                {
                    var syntax = Provider.GetService<ISyntaxProvider>();
                    ProcessPageEvaluator(context);

                    var newCommandText = syntax.Segment(context);
                    context.Command.CommandText = newCommandText;

                    InvokeSegmentCommand(acTrue);
                }
                catch (SegmentNotSupportedException)
                {
                    InvokeSegmentCommand(acTrue, acFalse);
                }
            }
            else
            {
                InvokeSegmentCommand(acTrue, acFalse, acNosegment);
            }
        }

        private void InvokeSegmentCommand(Action acTrue, Action acFalse = null, Action acNosegment = null)
        {
            if (acNosegment != null)
            {
                acNosegment();
            }
            else if (acFalse != null)
            {
                acFalse();
            }
            else if (acTrue != null)
        ...
        private void ProcessPageEvaluator(CommandContext context)
        {
            var evaluatable = context.Segment as IDataPageEvaluatable;
            if (evaluatable != null && evaluatable.Evaluator != null)
            {
                evaluatable.Evaluator.Evaluate(context);
            }
        }

        private void TrackFailedLog(DbCommand command, Exception exp)
        {
            if (ConnectionString.IsTracking && Track != null)
            {
                Track.Fail(command, exp);
