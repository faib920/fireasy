    ...
    public class TreeListIdentityDecorationRenderer : TreeListDefaultDecorationRenderer
    {
        public override void DrawCell(TreeListCellRenderEventArgs e)
        {
            ...
            if (item.Items.Count > 0 || item.ShowExpanded)
            {
                if (item.Expended)
                {
                    image = treeList.UseDefaultNodeImage ? Properties.Resources.tree_folder_open : treeList.DefaultExpandNodeImage;
                }
                else if (!item.Expended || item.ShowExpanded)
                {
                    image = treeList.UseDefaultNodeImage ? Properties.Resources.tree_folder : treeList.DefaultCollapseNodeImage;
                }
            }

            image = image ?? e.Cell.Item.GetImage() ?? (treeList.UseDefaultNodeImage ? Properties.Resources.tree_file : treeList.DefaultImage);
